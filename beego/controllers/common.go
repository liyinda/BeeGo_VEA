...
//跨域
func (self *BaseController) AllowCross() {
    self.Ctx.ResponseWriter.Header().Set("Access-Control-Allow-Origin", "http://101.200.42.56:8888")       //允许访问源
    self.Ctx.ResponseWriter.Header().Set("Access-Control-Allow-Methods", "POST, GET, PUT, OPTIONS")    //允许post访问
    self.Ctx.ResponseWriter.Header().Set("Access-Control-Allow-Headers", "Content-Type,Authorization,X-Token") //header的类型
    self.Ctx.ResponseWriter.Header().Set("Access-Control-Max-Age", "1728000")
    self.Ctx.ResponseWriter.Header().Set("Access-Control-Allow-Credentials", "true")
    self.Ctx.ResponseWriter.Header().Set("content-type", "application/json") //返回数据格式是json
}